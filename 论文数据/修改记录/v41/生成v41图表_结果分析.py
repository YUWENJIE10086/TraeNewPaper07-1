...
CHART_DIR = r'd:\TraePP06\论文数据\修改记录\v41'
with open(os.path.join(CHART_DIR, 'v41_分析结果.json'), encoding='utf-8') as f:
    R = json.load(f)
npz = np.load(os.path.join(CHART_DIR, 'v41_predictions.npz'), allow_pickle=True)
...
def plot_kernel_density():
    ...
    for title, data, ax, color in configs:
        ...
        add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_核密度分布.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_核密度分布')

def plot_model_r2():
    """图3: 四模型R2对比 - 指定配色，无外框线，R²正确显示"""
    md = ['LightGBM', '随机森林', '线性回归', '简单基线']
    v = [lgb_r2, rf_r2, lr_r2, base_r2]
    cs_bar = [C1, C2, C3, C4]
    fig, ax = plt.subplots(figsize=(10, 6))
    # 修改意见5: 无外框线(edgecolor='none')，R²用Unicode上标
    bars = ax.bar(md, v, color=cs_bar, alpha=0.90, edgecolor='none', width=0.55)
    for b, val in zip(bars, v):
        ax.text(b.get_x()+b.get_width()/2, b.get_height()+0.015, f'R2={val:.3f}',
                ha='center', fontsize=FS, fontweight='bold', fontfamily='SimHei')
    ax.set_ylabel('整体R2', fontsize=FS, fontfamily='SimHei')
    ax.tick_params(labelsize=FS-1); ax.set_ylim(0, 1.15)
    ax.grid(axis='y', alpha=0.15, linestyle='--'); ax.set_axisbelow(True)
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_四模型R2.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_四模型R2')
...
def plot_lgbm_evaluation():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_LightGBM评估.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_LightGBM评估')
...
def plot_quadrant():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_四象限.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_四象限')
...
def plot_grouped_bar():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_分组柱状图.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_分组柱状图')
...
def plot_radar():
    ...
    plt.savefig(os.path.join(CHART_DIR, 'v41_雷达图.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_雷达图')
...
def plot_risk_distribution():
    ...
    add_arrows(ax2)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_风险分布.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_风险分布')
...
def plot_risk_contribution():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v41_风险贡献度.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v41_风险贡献度')
