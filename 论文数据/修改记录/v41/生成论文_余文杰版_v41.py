...
修改要点:
1.摘要加"提前预警"意思
2.数据描述改为3/5/7/15/30天窗口预测
3.R²说明是什么+代表性参数+预警描述+为xxx提供支持
4.引言末段分层(1)编码(2)模型(3)风险评分(4)预警提示
5.1.1整体算法思想+多窗口
6.1.1只写数据来源预处理,1.2.1只写编码不重复
7.图1配色改进
8.图2根据v40技术路线
9.1.2.2删掉"参考姜思明等[3]"
10.全文下标检查
...
V41 = r'd:\TraePP06\论文数据\修改记录\v41'
ROOT = r'd:\TraePP06\论文数据'
with open(os.path.join(V41, 'v41_分析结果.json'), encoding='utf-8') as f:
    R = json.load(f)
...
mr(p_abs,
   '【背景和目的】针对烟草零售终端扫码数据丰富但缺乏前瞻性预测预警能力的问题，提出基于LightGBM的时序预测框架，预测未来终端运行指标并量化零售终端运行风险，实现提前预警。'
   '【方法】以兴山县133家零售终端扫码数据为样本，将运维办法7项指标映射为14个预测目标，分别按3、5、7、15和30天五种时间窗口进行预测，以各窗口的上期和同期数据为特征训练模型，以LightGBM为主模型，对比随机森林、线性回归和简单基线四种模型。'
   f'【结果】LightGBM在所有指标上均表现最优，整体R²={lgb_r2:.3f}，精确率-召回率曲线下面积达{clf["pr_auc"]:.4f}，ROC曲线下面积达{clf["roc_auc"]:.4f}。'
   '【结论】构建0至100分零售终端运行风险评分四级预警机制，可预警终端经营下滑、扫码异常、支付萎缩等风险，为烟草零售终端的运行监测、客户经理重点走访排序和客服部货源投放调整提供了数据驱动的决策支持。',
   cf='宋体', fs=14)
...
# 修改意见4: 引言末段分层(1)编码(2)模型(3)风险评分(4)预警提示
ap(
    f'本文以兴山县{N}家零售终端2026年1至5月扫码数据为基础，提出基于LightGBM的时序预测预警框架：'
    '（1）采用上月趋势加历史同期的双维时序特征编码方案，将运维办法7项指标映射为14个预测目标；'
    '（2）以LightGBM为核心模型，对比线性回归、随机森林和简单基线共四种模型，采用决定系数R²衡量回归精度，以精确率-召回率曲线、ROC曲线和四象限分析验证分类效果；'
...
# 修改意见5+6: 1.1只写数据来源和预处理，整体算法思想含多窗口
...
ap(
    '整体预测思路为：以各时间窗口的上期数据和去年同期数据作为特征，该窗口实际数据作为预测目标，训练LightGBM等模型进行预测。'
    '时间窗口设为3天、5天、7天、15天和30天五种，灵活适配不同管理场景的时间粒度需求：3天窗口支持应急排查，7天窗口对应周评价，15天窗口适配订货调整，30天窗口对应月度考核。'
    '以30天窗口为例，预测2月数据时以1月数据和2025年2月数据为特征输入，2月实际数据为预测目标；预测3月则以2月数据和2025年3月数据为特征，以此类推。'
    '其他窗口同理，如7天窗口以过去7天和去年同期7天数据为特征，预测未来7天的指标。'
)
...
# 修改意见6: 1.2.1只写编码，不重复数据来源
ahead('1.2.1  时序特征编码方案', 3)
ap(
    '特征编码采用双维时序方案，核心思想是利用"上月趋势+历史同期"两个维度的信息预测目标期指标。'
    '维度A为上期特征：取目标期的上一期数据共14维，反映终端近期的经营趋势和状态变化；'
    '维度B为同期特征：取去年同期的数据共14维，捕捉终端的季节性消费规律。'
)
ap(
    '以30天窗口预测2月数据为例：上期取1月数据，1月是距离2月最近的完整月度数据，包含终端最新的经营状态；'
    '同期取2025年2月数据，去年同期反映了该月的季节性消费规律，如节假日消费高峰、季节性品规需求变化等。'
    '同理，预测5月数据则取4月数据和2025年5月数据为特征。'
    '两者结合既能捕捉近期动态变化，又能融入历史同期规律，从而更准确地预测目标期指标。'
    '此外，利用已有数据构建趋势均值共14维，同比变化率14维，合计56维输入特征。'
    '该编码方案同样适用于3天、5天、7天和15天窗口，只需将月度数据替换为对应窗口长度的数据即可。'
)
ap('图1展示了核心指标的核密度分布。日均扫码笔数和在线支付笔数均呈现明显的右偏分布，'
   '说明大部分终端扫码量和支付量偏低，少数终端贡献了主要交易量。'
   '使用率分布相对集中，大部分终端使用率在60%至90%之间。',
   indent=True)
afig(f'{V41}/v41_核密度分布.png',
...
p_nb.paragraph_format.space_after = Pt(1)
...
p_nb2 = doc.add_paragraph(); p_nb2.paragraph_format.line_spacing = 1.5
p_nb2.paragraph_format.first_line_indent = Pt(28)
oMP_nb2 = OxmlElement('m:oMathPara'); oM_nb2 = OxmlElement('m:oMath')
oM_nb2.append(m_sub('y', 'j,T')); oM_nb2.append(m_run_ni('为第j项指标第T期的实际值。该基线衡量机器学习模型相比朴素预测的增量价值。'))
oMP_nb2.append(oM_nb2)
p_nb2._element.append(oMP_nb2)
...
# 修改意见11: ROC-AUC名称调整 - 不用"受试者工作特征"，用更通用的"ROC曲线"
ap(
    '（5）精确率-召回率曲线下面积（Precision-Recall Area Under Curve，PR-AUC）。以召回率为横轴、精确率为纵轴绘制曲线，曲线下面积越大说明高风险终端识别效果越好，'
    '在正负样本不均衡时比ROC曲线下面积更能反映模型性能。',
    indent=True, sa=2)
ap(
    '（6）ROC曲线下面积（Receiver Operating Characteristic Area Under Curve，ROC-AUC）。ROC曲线以假阳性率为横轴、真阳性率为纵轴绘制，曲线下面积越接近1说明模型区分能力越强。'
...
ap('')
ap(
    f'由表4可以看出，LightGBM整体R²={lgb_r2:.3f}最高，较随机森林R²={rf_r2:.3f}提升{lgb_r2-rf_r2:.3f}，'
    f'较线性回归R²={lr_r2:.3f}提升{lgb_r2-lr_r2:.3f}，基线模型仅为{base_r2:.3f}。'
    f'精确率-召回率曲线下面积达{clf["pr_auc"]:.4f}，ROC曲线下面积达{clf["roc_auc"]:.4f}，四象限一致性{clf["consistency"]:.1%}，'
    '在回归精度和分类判别能力上均优于其他模型。'
    '梯度提升的序列化纠错机制使其在多目标预测中优于Bagging的并行投票机制和线性模型的简单可加假设。'
    f'图3直观展示了四模型R²的差距。'
)
afig(f'{V41}/v41_四模型R2.png',
     '图3  四模型整体R²对比',
     f'LightGBM整体R²={lgb_r2:.3f}最高，随机森林次之，基线模型最低。')
ap(
    '图4为LightGBM的分类评估曲线。子图a为精确率-召回率曲线，曲线下面积接近0.93，表明高风险终端识别效果出色；'
    '子图b为ROC曲线，远离对角线，说明整体判别能力强。'
    '图5为四象限一致性分析，大部分终端落在Q1和Q3正确预测区域。'
)
afig(f'{V41}/v41_LightGBM评估.png',
     '图4  LightGBM分类评估（精确率-召回率曲线/ROC曲线）',
     '两子图从不同维度综合评估LightGBM的分类性能。')
afig(f'{V41}/v41_四象限.png',
     '图5  四象限一致性分析',
...
p_sdev = doc.add_paragraph(); p_sdev.paragraph_format.line_spacing = 1.5
p_sdev.paragraph_format.first_line_indent = Pt(28)
oMP_sd = OxmlElement('m:oMathPara'); oM_sd = OxmlElement('m:oMath')
oM_sd.append(m_sub('S', 'dev')); oM_sd.append(m_run_ni('为预测偏差得分，基于预测值与基期值的相对偏差计算。'))
oMP_sd.append(oM_sd)
p_sdev._element.append(oMP_sd)
...
ap('表5  LightGBM零售终端运行风险评分四级分布', cf='宋体', fs=10.5, align=WD_ALIGN_PARAGRAPH.CENTER, indent=False, sa=2)
t5 = doc.add_table(rows=5, cols=5)
t5.alignment = WD_TABLE_ALIGNMENT.CENTER; make_three_line_table(t5)
for j, h in enumerate(['运行风险等级', '评分阈值', '终端数', '占比/%', '响应策略']): sc(t5.rows[0].cells[j], h, fs=10.5, bd=True)
total_s = rd['red'] + rd['orange'] + rd['yellow'] + rd['normal']
for i, row_data in enumerate([
    ['红色预警', '>=60', 'red'], ['橙色预警', '40至59', 'orange'],
    ['黄色预警', '20至39', 'yellow'], ['正常', '<20', 'normal'],
]):
    cnt = rd.get(row_data[2], 0)
    sc(t5.rows[i+1].cells[0], row_data[0], fs=10.5); sc(t5.rows[i+1].cells[1], row_data[1], fs=10.5)
    sc(t5.rows[i+1].cells[2], str(cnt), fs=10.5)
    sc(t5.rows[i+1].cells[3], f'{cnt/total_s*100:.1f}', fs=10.5)
    sc(t5.rows[i+1].cells[4], ['立即实地走访', '本周期走访', '加强线上监测', '常规监测'][i], fs=10.5)
ap('')
ap(
    f'预警精确率{warn_prec:.4f}，四象限一致性{clf["consistency"]:.1%}。图6展示了评分分布，图7展示了各指标的零售终端运行风险贡献度。'
)
afig(f'{V41}/v41_风险分布.png',
     '图6  零售终端运行风险综合评分分布',
     '饼图为等级分布，直方图为评分分布，虚线为四级阈值。')
afig(f'{V41}/v41_风险贡献度.png',
     '图7  各指标零售终端运行风险贡献度',
     '日均扫码笔数和在线支付笔数合计57%，为主要运行风险驱动因素。')

ahead('3.2  多层级决策应用', 2)
ap('基于零售终端运行风险评分结果，构建三级管理层决策体系：')
# 修改意见3: 预警不说多少家，说预警哪些到什么程度
ap(
...
outpath = os.path.join(V41, '基于LightGBM的零售终端时序预测与预警研究_v41b.docx')
outpath_tmp = os.path.join(V41, '基于LightGBM的零售终端时序预测与预警研究_v41c.docx')
try:
    doc.save(outpath)
    print(f'[OK] 论文v41已保存: {outpath}')
except PermissionError:
    doc.save(outpath_tmp)
    print(f'[OK] 论文v41已保存(临时): {outpath_tmp}')
    outpath = outpath_tmp
