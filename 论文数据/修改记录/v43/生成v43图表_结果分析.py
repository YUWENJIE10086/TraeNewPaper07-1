"""
v43图表 - 在v42基础上改进:
- 图6饼图可读性大幅优化: 更大图幅、加粗黑色百分比、环形图、外部图例、更鲜艳配色
- 其余图表保持v42风格(8色方案、箭头、无标题、edgecolor='none'、R²上标等)
"""
import matplotlib
matplotlib.use('Agg')
...
CHART_DIR = r'd:\TraePP06\论文数据\修改记录\v43'
with open(os.path.join(CHART_DIR, 'v43_分析结果.json'), encoding='utf-8') as f:
    R = json.load(f)
npz = np.load(os.path.join(CHART_DIR, 'v43_predictions.npz'), allow_pickle=True)
...
def plot_kernel_density():
    ...
    for title, data, ax, color in configs:
        ...
        add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_核密度分布.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_核密度分布')
...
def plot_model_r2():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_四模型R2.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_四模型R2')
...
def plot_lgbm_evaluation():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_LightGBM评估.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_LightGBM评估')
...
def plot_quadrant():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_四象限.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_四象限')
...
def plot_grouped_bar():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_分组柱状图.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_分组柱状图')
...
def plot_radar():
    ...
    ax.set_ylim(0.55, 1.08); ax.tick_params(labelsize=FS-2)
    # 加粗所有模型线型和标记，确保封口可见
    for line in ax.lines:
        line.set_linewidth(2.2); line.set_markersize(7)
    ...
    plt.savefig(os.path.join(CHART_DIR, 'v43_雷达图.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_雷达图')

def plot_risk_distribution():
    """图6: 风险分布 - 饼图可读性大幅优化: 环形图、黑色加粗百分比、更大图幅、外部图例"""
    # 更大图幅，饼图占更多空间
    ...
    for t in texts:
        ...
        t.set_fontweight('bold')
    # 外部图例，更大字体
    ax1.legend(wedges, [f'{l} ({s}家)' for l, s in zip(labels_pie, sizes)],
               loc='center left', bbox_to_anchor=(-0.25, 0.5),
               fontsize=FS+1, framealpha=0.95, edgecolor='#cccccc',
               title='风险等级', title_fontsize=FS+2)
    ...
    add_arrows(ax2)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_风险分布.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_风险分布')
...
def plot_risk_contribution():
    ...
    add_arrows(ax)
    plt.tight_layout()
    plt.savefig(os.path.join(CHART_DIR, 'v43_风险贡献度.png'), dpi=200, bbox_inches='tight')
    plt.close(); print('[OK] v43_风险贡献度')
