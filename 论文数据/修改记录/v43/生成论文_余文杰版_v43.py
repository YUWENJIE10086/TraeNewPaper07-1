...
v43论文 - 格式模板合规修订（续）
修改要点:
1.文件路径改为v43，数据文件v43_分析结果.json
2.图表文件名改为v43_前缀
3.结论(Section 4)重写：按格式模板要求，(1)新发现新观点 (2)本质和规律性认识 (3)理论意义和应用价值 (4)进一步研究方向
4.标题LightGBM保留(专有名词)，摘要确认无非公知公用符号
5.正文对每个图都有简单叙述
6.检查图表格式规则(15-20)合规
7.作者标注改为多作者+上标+双单位
8.输出文件名v43，PermissionError回退_v43b
"""
import os, json
...
# v43修改：作者标注改为多作者+上标编号+双单位
# 格式模板：作者仿宋体四号(14pt)，不同单位用上标数字区分
...
ahead('0引言')
ap(
    '湖北烟草"知音通"零售终端管理系统推广以来，兴山县零售终端积累了扫码记录、在线时长等多维度运行数据[1]。'
    '现行运维管理办法建立了覆盖7项指标的周评价机制，满分100分。'
    '然而该机制基于已发生数据事后计算得分，无法回答下个周期指标如何变化。'
...
ap('')
ap(
    '姜思明等[2]提出"数据治理—状态评估"一体化框架，采用K-means++聚类剔除异常终端。'
    '李天举等[3]基于Stacking集成学习进行烟草异常数据挖掘，查实率提升至94.3%。'
    'Grinsztajn等[4]和Borisov等[5]证实GBDT类方法在表格数据建模上优于深度学习方法。'
    '然而现有研究多聚焦事后评估，对多指标同步预测和前瞻性预警关注较少。'
)
...
# v43修改：正文对图3有简单叙述
ap(f'图3直观展示了四模型R²的差距，LightGBM整体R²最高，随机森林次之，基线模型最低。')
afig(f'{V43}/v43_四模型R2.png',
     '图3  四模型整体R²对比',
...
# (4) 进一步研究方向
ap(
    '（4）后续研究方向：引入更多月份数据构建多步滚动预测框架；'
    '参考姜思明等[2]的LFM方法融合经验权重与数据驱动权重；'
    '结合SHAP可解释性分析提供个性化预警原因诊断；'
    '增加库存管理类指标使指标体系更完整。',
    indent=True)
...
outpath = os.path.join(V43, '基于LightGBM的零售终端时序预测与预警研究_v43.docx')
outpath_bak = os.path.join(V43, '基于LightGBM的零售终端时序预测与预警研究_v43b.docx')
try:
    doc.save(outpath)
    print(f'[OK] 论文v43已保存: {outpath}')
except PermissionError:
    doc.save(outpath_bak)
    print(f'[OK] 论文v43已保存(备用): {outpath_bak}')
    outpath = outpath_bak
