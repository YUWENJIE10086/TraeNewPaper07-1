# -*- coding: utf-8 -*-
"""v20最终一致口径实验管线 0916_0827：P35按ym当月横截面计算。"""
...
def monthly_rank_score(d):
    ...
    d['Sstar']=100*np.column_stack(parts).mean(axis=1)
    return d
...
def prepare():
    ...
    # 正式口径：每个ym独立计算当月横截面P35，不跨年份合并同月份。
    q35=d.groupby('ym')['fill'].transform(lambda x:x.quantile(.35))
...
def topk(p):
    ...
    for pct in [10,20,30,40,50]:
        ...
        b=p.nlargest(n,'lag_inventory_ratio'); r['inventory_dr']=b.dr_score.mean(); r['gain_vs_inventory']=a.dr_score.mean()-b.dr_score.mean(); rows.append(r)
