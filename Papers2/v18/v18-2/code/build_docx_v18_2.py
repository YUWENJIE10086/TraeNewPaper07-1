...
BASE = Path(r"D:\TraeNewPaper07-1\Papers2\v18\v18-2")
HTML = BASE / "论文v18-2.html"
TEMPLATE = Path(r"D:\TraeNewPaper07-1\Papers2\v18\v18-2\rendered\论文模板.docx")
OUT = BASE / "论文v18-2.docx"
CONTENT_W = 15.92  # cm
...
def F_v18_4():
    """v18-2 第(4)式：τ̂(x)=arg min_τ Σ_{i∈L(x)} α_i(x)(Ŷ_i−τ·T̃_i)^2"""
    inner = (om_sub(om_acc(om_i("Y"), "ˆ"), om_i("i")) + "−" + om_i("τ") + om_run("·") +
             om_sub(om_acc(om_i("T"), "˜"), om_i("i")))
    body = (om_sub(om_i("α"), om_i("i")) + om_paren(om_i("x")) +
            om_sup(om_paren(inner), om_run("2")))
    return (om_sub(om_acc(om_i("τ"), "ˆ"), om_paren(om_i("x"))) + "=" +
            om_sub(om_run("arg min"), om_i("τ")) +
            om_sum(om_i("i∈L(x)"), om_run(""), body))
...
def build():
    ...
    # v18-2 预处理：figcaption→div.figcap；span class="rn"→"n"；title 类名规整
    ...
    article = article.replace('<h1 class="title"', '<h1 class="paper-title"')
