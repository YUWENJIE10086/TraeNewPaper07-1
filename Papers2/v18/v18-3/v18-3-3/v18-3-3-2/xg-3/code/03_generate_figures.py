...
def fig_density_advanced():
    """复刻母版分组密度图；边界效应采用直方密度平滑，避免KDE越界。在图中标注各组 n/均值/中位数统计框（对应图表-02）。"""
    ...
    for g in GROUP_ORDER:
        ...
        stat_lines.append(f"{g}：n={vals.size}，均值={np.mean(vals):.3f}，中位数={np.median(vals):.3f}")
    ...
    box = "\n".join(stat_lines)
    ax.text(.02, .99, box, transform=ax.transAxes, fontsize=8.2, va="top", ha="left",
            bbox=dict(boxstyle="round,pad=.45", fc="white", ec="#9fb1c6", alpha=.92))
