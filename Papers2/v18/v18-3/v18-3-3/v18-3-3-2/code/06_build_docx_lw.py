# -*- coding: utf-8 -*-
"""论文 v18-3 → DOCX 构建脚本（公式为标准 OMML 可编辑 Word 公式）。

解析 论文v18-3.html，生成符合《中国烟草学报》排版规范的 DOCX：
- 标题/摘要/Keywords、双语图注/表注
- 一~三级标题（黑体、顶格）、正文（宋体、首行缩进）
- 公式：Word 原生 OMML，可在 Word 中点击为"公式"再编辑
- 三线表、插图（居中）、参考文献、右下角页码
"""
import re
...
BASE = Path(r"D:\TraeNewPaper07-1\Papers2\v18\v18-3\v18-3-3\v18-3-3-1")
HTML = BASE / "v18-3-3-1lw.html"
TEMPLATE = BASE / "rendered" / "论文模板.docx"
OUT = BASE / "v18-3-3-1lw.docx"
CONTENT_W = 15.92  # cm
...
def F10():
    return (om_sub(om_i("I"), om_i("j")) + "=" +
            om_frac(om_sub(om_i("G"), om_i("j")),
                    om_sum(om_i("k=1"), om_i("p"), om_sub(om_i("G"), om_i("k")))))


def F11():
    """v18-3 第(11)式：max Σ_r I_r(c̄_r+τ̂(x_r))c_r, s.t. Σ_r c_r≤C, 0≤c_r≤c̄_r"""
    cr = om_sub(om_i("c"), om_i("r"))
    cbar = om_acc(cr, "¯")
    tx = om_sub(om_acc(om_i("τ"), "ˆ"), om_paren(om_sub(om_i("x"), om_i("r"))))
    term = om_sub(om_i("I"), om_i("r")) + om_paren(cbar + "+" + tx) + cr
    left = om_run("max ") + om_sum(om_i("r=1"), om_i("R"), term)
    csum = om_sum(om_i("r=1"), om_i("R"), cr)
...
class BlockParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        attrs = dict(attrs)
        cls = attrs.get("class", "")
        if tag == "section" and cls == "abstract":
            self.in_abstract = True
        elif tag == "div" and cls == "ref":
            self.in_ref = True
    ...
    def handle_endtag(self, tag):
        if tag in {"td", "th"} and self.cell is not None:
            ...
            html = "".join(self.raw)
            cls = self.in_p[1]
            if cls == "" and self.in_abstract:
                cls = "abstract"
            if cls == "" and self.in_ref:
                cls = "ref"
            self.blocks.append((self.in_p[0], cls, html))
            self.in_p = None
        elif tag == "section":
            self.in_abstract = False
        elif tag == "div" and self.in_ref:
            self.in_ref = False
...
def add_ref_para(doc, html, size=9.5, east="宋体"):
    p = doc.add_paragraph()
    p.paragraph_format.line_spacing = 1.25
    p.paragraph_format.space_after = Pt(2)
    p.paragraph_format.left_indent = Pt(size * 2)
    p.paragraph_format.first_line_indent = Pt(-size * 2)
    ...
    for i, part in enumerate(parts):
        ...
        add_rich_text(p, Inline().parse(part), base_size=size, east=east)
    return p
...
def build():
    ...
    article = re.sub(r"<figcaption>(.*?)</figcaption>", r'<div class="figcap">\1</div>', article, flags=re.S)
    article = re.sub(r'<span class="rn">', '<span class="n">', article)
    ...
    while i < len(bl):
        ...
        if kind == "h1":
            ...
            cls, html = rest
            if cls in {"paper-subtitle", "meta", "en"}:
