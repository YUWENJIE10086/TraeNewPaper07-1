...
DATA = r"D:\TraeNewPaper07-1\Papers2\v18\v18-3\v18-3-3\v18-3-3-2\data"
FIG  = r"D:\TraeNewPaper07-1\Papers2\v18\v18-3\v18-3-3\v18-3-3-2\figures"
...
def fig2():
    ...
    for x, c, txt in [(np.mean(tau), C_GRAY, f"ATE={np.mean(tau):.2f}"), (q_lo, C_LIGHT, f"低敏感上界={q_lo:.2f}"), (q_hi, C_ACCENT, f"高敏感下界={q_hi:.2f}")]:
...
def fig8():
    ...
    im = ax.imshow(arr, cmap="RdBu", aspect="auto")
    ...
    ax.set_xlabel("排序组别")
    ax.set_ylabel("价位段")
    chart_title(ax, "不同价位段在不同排序组中的信号分化")
    ...
    ax.text(0.02, -0.16, "注：单元格数值为该价位段在对应排序组中的平均条件关联信号，括号内为样本量；颜色越红信号越高，越蓝越低。", transform=ax.transAxes,
...
def fig10():
    ...
    if "name" not in sdp.columns or sdp["name"].isna().all():
        nm_col = data[["code", "name"]].drop_duplicates("code")
        sdp = sdp.merge(nm_col, on="code", how="left")
    ...
    _nm = str(sdp["name"].iloc[0]) if "name" in sdp.columns and len(sdp) else code
    chart_title(ax, f"高敏感代表品规「{_nm}」月度状态轨迹")
    fig.tight_layout()
...
def fig12():
    ...
    hi_names = name_rank["name"].dropna().drop_duplicates().head(4).tolist()
    lo_names = name_rank.sort_values(["mean_cate", "treat_cnt"], ascending=[True, False])["name"].dropna().drop_duplicates().head(4).tolist()
    ...
    fig, axes = plt.subplots(4, 2, figsize=(11.6, 13.2), sharex=False, sharey=True)
    axes = axes.ravel()
    hi_set = set(hi_names)
    ...
    for ax in axes[len(names_pick):]:
        ax.axis("off")
    fig.text(0.5, 0.958, "高敏感代表样本", ha="center", va="top", fontsize=10.4, color=C_PRIMARY, weight="bold")
    ...
    fig.suptitle("8个代表性品规月度综合评分轨迹与供给收缩事件对照\n（橙色竖带=供给收缩事件）", fontsize=11.6, color=C_DARK, y=0.995, weight="bold")
...
def fig_density_advanced():
    """按CATE三分位排序组的密度分布；附每组样本量与集中趋势标注。"""
    fig, ax = plt.subplots(figsize=(7.6, 5.4))
    colors = {"低排序组": C_LIGHT, "中排序组": C_SECOND, "高排序组": C_ACCENT, "总体": "#B4555B"}
    lo, hi = float(data.CATE.min()), float(data.CATE.max())
    bins = np.linspace(lo, hi, 34); centers = (bins[:-1] + bins[1:]) / 2
    # 组样本量与统计量（按sensitivity列名映射到排序组名）
    stats = []
    for g, gname in [("低敏感", "低排序组"), ("中敏感", "中排序组"), ("高敏感", "高排序组")]:
        vals = data.loc[data.sensitivity == g, "CATE"].to_numpy(float)
        hist, _ = np.histogram(vals, bins=bins, density=True)
        smooth = np.convolve(hist, np.ones(3) / 3, mode="same")
        ax.fill_between(centers, 0, smooth, step="mid", color=colors[gname], alpha=.16)
        ax.plot(centers, smooth, color=colors[gname], lw=1.8, label=gname)
        med = np.median(vals)
        ax.axvline(med, color=colors[gname], lw=.8, alpha=.75)
        ax.annotate("" if len(vals) < 1 else "", xy=(med, ax.get_ylim()[1]*0.05))
        stats.append(f"{gname} n={len(vals)}  均值={vals.mean():.2f}  中位数={med:.2f}")
    ...
    stats_box = "\n".join(stats) + f"\n总体 n={len(vals)}  均值={vals.mean():.2f}  中位数={np.median(vals):.2f}"
    ax.text(0.02, 0.97, stats_box, transform=ax.transAxes, va="top", fontsize=7.2,
            family="sans-serif", bbox=dict(boxstyle="round,pad=0.35", fc="#F7F8FA", ec="#C4CEDA", lw=0.7))
    ax.set_xlabel("条件关联信号 / CAS score"); ax.set_ylabel("密度 / Density"); set_style(ax)
    ax.legend(frameon=False, fontsize=9, loc="upper right")
    chart_title(ax, "CATE高/中/低排序组的条件关联信号分布")
...
def fig_absorption_advanced():
    ...
    ax.text(.02,.96,"高顺价动能—高库存缓解\nCATE高，高排序复核",transform=ax.transAxes,fontsize=8.5,color=C_PRIMARY,va="top")
    ax.text(.67,.08,"修复动能弱—库存承压\n转向动销观察",transform=ax.transAxes,fontsize=8.5,color=C_SECOND,va="bottom")
    ax.set_xlabel("顺价与需求修复动能（标准化）"); ax.set_ylabel("库存缓解与动销承接（标准化）")
    set_style(ax); cb=fig.colorbar(sc,ax=ax,fraction=.046,pad=.03); cb.set_label("品规层条件关联信号 / CAS")
    chart_title(ax,"缩投前经营画像与CATE排序分布")
    fig.tight_layout(); fig.savefig(os.path.join(FIG,"fig4_effect_absorption_map.png"),bbox_inches="tight"); plt.close(fig)
