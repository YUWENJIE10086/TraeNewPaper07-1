...
DATA = r"D:\TraeNewPaper07-1\Papers2\v18\v18-4\data"
FIG  = r"D:\TraeNewPaper07-1\Papers2\v18\v18-4\figures"
...
# ---------- 读数据 ----------
data = pd.read_csv(os.path.join(DATA, "causal_forest_panel.csv"), encoding="utf-8-sig")
...
def fig2():
    ...
    ax1_r.set_ylabel("核密度", color=C_SECOND, fontsize=9.8)
    ...
    ax2.set_ylabel("累计占比")
    ax2.set_xlabel("条件平均处理效应 CATE（供给收缩对下月状态评分变化的影响）")
    ax2.set_ylim(0, 1.02)
    set_style(ax2)
...
def fig_absorption_advanced():
    ...
    ax.set_xlabel("市场连接与供给嵌入能力（标准化综合得分）"); ax.set_ylabel("资源调度与状态吸收能力（标准化综合得分）")
...
def fig_subgroup_forest_advanced():
    ...
    ax.set_xlabel("平均CATE及95%置信区间"); set_style(ax); ax.grid(axis="x",ls="--",lw=.55,alpha=.25); chart_title(ax,"价位段、品类与时令异质性森林图")
    fig.tight_layout(); fig.savefig(os.path.join(FIG,"fig6_subgroup_forest.png"),bbox_inches="tight"); plt.close(fig)

if __name__ == "__main__":
    fig2(); fig_density_advanced(); fig_absorption_advanced(); fig8(); fig_subgroup_forest_advanced(); fig10(); fig12()
